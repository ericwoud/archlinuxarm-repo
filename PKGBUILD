# Arch64 kernel for OLIMEX AM335x EVB board
...
pkgver=r6.14.0.65d7dd6a0b1c.dirty.65d7dd6a0b
pkgrel=1
arch=('armv7h' 'x86_64')
...
if [[ "$(uname -m)" != "armv7l" ]]; then
  makedepends+=(arm-none-linux-gnueabihf-toolchain-bin)
  export MAKEFLAGS+=" ARCH=arm CROSS_COMPILE=arm-none-linux-gnueabihf-"
fi
...
build() {
  ...
  make ${MAKEFLAGS} $_llvm zImage modules # KCFLAGS=-w
...
_package() {
  ...
  install=linux-am335x-git.install
  ...
  install -Dt "${pkgdir}/boot"      -m600 arch/$KARCH/boot/zImage
  install -Dt "${pkgdir}/boot/dtbs" -m600 arch/$KARCH/boot/dts/ti/omap/am335x-olimex-*.dtb
